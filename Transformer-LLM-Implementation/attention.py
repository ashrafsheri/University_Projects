...
class RotaryEmbedder(nn.Module):
    ...
    def forward(self, x):
        ...
        frequency = t.unsqueeze(1) * self.freq.to(device).unsqueeze(0)
        emb = torch.cat([frequency, frequency], dim=-1).to(dtype) 
        cos = emb.cos()
        sin = emb.sin()
        return cos, sin
...
class GroupedQueryAttention(nn.Module):
    ...
    def forward(self, x: torch.Tensor, attention_mask=None):
        ...
        k = k.view(batch, seq_len, self.kv_heads, self.head_dim).permute(0, 2, 1, 3)
        v = v.view(batch, seq_len, self.kv_heads, self.head_dim).permute(0, 2, 1, 3)
        ...
        z = torch.arange(seq_len, device=x.device).unsqueeze(1)
        
        causal_mask = (z >= j).float()[None, None, :, :]
        scores = scores.masked_fill(causal_mask == 0, float('-inf'))

        probabilities = F.softmax(scores, dim=-1)
        attention_output = torch.matmul(probabilities, v)
        attention_output = attention_output.permute(0, 2, 1, 3).contiguous()
        attention_output = attention_output.view(batch, seq_len, self.hidden_size)
